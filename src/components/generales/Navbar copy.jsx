  ...
  return (
    <nav className="fixed w-full z-20 top-0 start-0 border-b border-marca-principal text-texto dark:text-texto-oscuro bg-fondo dark:bg-fondo-oscuro">
      ...
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
        ...
        {/* Menú principal */}
        <div className={`${menuAbierto ? 'block' : 'hidden'} items-center justify-between w-full md:flex md:w-auto`} id="navbar-sticky">
          <ul className="flex flex-col p-4 md:p-0 m-4 font-medium rounded-lg md:space-x-8 md:flex-row">
            {enlaces.map((enlace, index) => (
              <li key={index} className="relative">
                {'opciones' in enlace ? (
                  ...
                  <a
                    href={enlace.url}
                    className="block py-2 px-3 hover:bg-marca-principal hover:text-white rounded-lg dark:hover:bg-oscuro-destacado"
                  >
                    {enlace.texto}
                  </a>
                )}
            ...
            {mostrarSelectorDeTema && (
              <li id='mostrarSelectorDeTema' className="mt-6 flex justify-end md:hidden">
                <SelectorDeTema />
              </li>
            )}
           </ul>
